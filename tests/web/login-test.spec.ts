...
test.describe.serial("Login Page Tests", () => {
  ...
  testSetup("Verify error displayed for invalid credentials", async ({ loginPage }) => {
    await loginPage.enterInvalidUsername(invalidUser);
    await loginPage.enterInvalidPassword(invalidPassword);
  ...
  testSetup("Verify error displayed when username is empty", async ({ loginPage }) => {
    await loginPage.enterValidPassword(validPassword);
    await loginPage.clickLoginButton();
    await loginPage.verifyErrorMessage("usernameRequired");
  });

  testSetup("Verify error displayed when password is empty", async ({ loginPage }) => {
    await loginPage.enterValidUsername(validUser);
    await loginPage.clickLoginButton();
    await loginPage.verifyErrorMessage("passwordRequired");
  });

  testSetup("Verify successful login with valid username & password", async ({ loginPage }) => {
    await loginPage.enterValidUsername(validUser);
    await loginPage.enterValidPassword(validPassword);
    await loginPage.clickLoginButton();
